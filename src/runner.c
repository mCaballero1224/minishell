...
{
  int flags = 0;
  /* Select the appropriate file flags.
   *
...
do_io_redirects(struct command *cmd)
{
  ...
  for (size_t i = 0; i < cmd->io_redir_count; ++i) {
    ...
    file_open:;
       ...
       */
      int src = open(r->filename, flags, (mode_t) 0777);
      if (src == -1) {
        perror("open");
        goto err;
      }
...
run_command_list(struct command_list *cl)
{
  ...
  for (size_t i = 0; i < cl->command_count; ++i) {
     ...
     *
     * Update upstream_pipefd initializer to get the (READ) side of the
    ...
    if (child_pid == 0) {
      if (is_builtin) {
         ...
         *
         * move upstream_pipefd to STDIN_FILENO  if it's valid
         *
         * move downstream_pipefd to STDOUT_FILENO if it's valid
         */
